...
一般只有在带副作用的函数里才有`try-catch`异常处理，**异常处理是个垃圾机制**，我不喜欢`try-catch`异常机制，使用`try-catch`异常机制也只是为了兼容系统外部的库、语言和平台的异常机制而已，对外来的只能接受，只能管好自己。**在数据流里，一切用数据解决**，可以接着向下传递数据信号表达状况，这种思想类似C语言的错误号数据传递机制。经典的计算机技术很多是很好的，比如数据结构+算法，错误号等。一帮不知所谓的家伙炒作出FP/00/异常等一堆垃圾，为创新而创新，为paper而创新。
